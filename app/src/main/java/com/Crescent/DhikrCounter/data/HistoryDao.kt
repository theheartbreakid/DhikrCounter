...
interface HistoryDao {
    ...
    @Query("SELECT SUM(countChange) FROM history WHERE sessionId = :sessionId AND eventType IN ('INCREMENT', 'DECREMENT', 'COUNT_CHANGED')")
    suspend fun getLifetimeCount(sessionId: Long): Long?

    @Query("SELECT SUM(countChange) FROM history WHERE sessionId = :sessionId AND timestamp >= :startTime AND eventType IN ('INCREMENT', 'DECREMENT', 'COUNT_CHANGED')")
    suspend fun getCountSince(sessionId: Long, startTime: Long): Long?

    @Query("SELECT SUM(countChange) FROM history WHERE sessionId = :sessionId AND timestamp >= :startTime AND timestamp < :endTime AND eventType IN ('INCREMENT', 'DECREMENT', 'COUNT_CHANGED')")
    suspend fun getCountInRange(sessionId: Long, startTime: Long, endTime: Long): Long?

    @Query("SELECT SUM(countChange) FROM history WHERE timestamp >= :startTime AND timestamp < :endTime AND eventType IN ('INCREMENT', 'DECREMENT', 'COUNT_CHANGED')")
    ...
    @Query("SELECT DISTINCT (timestamp / 86400000) FROM history WHERE sessionId = :sessionId AND eventType IN ('INCREMENT', 'COUNT_CHANGED') ORDER BY timestamp DESC")
    suspend fun getActiveDays(sessionId: Long): List<Long>

    @Query("SELECT SUM(countChange) FROM history WHERE eventType IN ('INCREMENT', 'DECREMENT', 'COUNT_CHANGED')")
    ...
    @Query("SELECT DISTINCT (timestamp / 86400000) FROM history WHERE eventType IN ('INCREMENT', 'COUNT_CHANGED') ORDER BY timestamp DESC")
